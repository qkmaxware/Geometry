# Creating Geometry
Qkmaxware.Geometry takes a simple approach to 3D geometry. Geometry is represented as a collection of triangles in which each triangle is defined by 3 vertices in 3D space. The `Qkmaxware.Geometry.Mesh` class is the base class for most of the operation on 3D geometry.
